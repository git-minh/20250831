  ...
  test.describe('Theme and Styling', () => {
    ...
    test('should match component styling consistency', async ({ page }) => {
      ...
      for (const selector of formElements) {
        await expect(page.locator(selector).first()).toHaveScreenshot(`element-${selector.replace(/[^a-z0-9]/gi, '-')}.png`);
      }
    });
    ...
    test('should match focus states', async ({ page }) => {
      ...
      await expect(page.locator('input[name="password"]')).toHaveScreenshot('password-input-focused.png');
      
      // Focus submit button
      await page.focus('button[type="submit"]');
      await expect(page.locator('button[type="submit"]')).toHaveScreenshot('submit-button-focused.png');
    });
    ...
    test('should match hover states', async ({ page }) => {
      ...
      await expect(page.locator('button[type="submit"]')).toHaveScreenshot('submit-button-hovered.png');
