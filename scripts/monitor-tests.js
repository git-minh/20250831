...
class TestMonitor {
  ...
  reportProgress() {
    ...
    this.log(
      `📊 Progress: ${totalTests} total, ${passedTests} passed, ${failedTests} failed, ${skippedTests} skipped (${failureRate.toFixed(1)}% failure rate)`
    );
  }
  ...
  checkAlerts(result) {
    ...
    // Check failure rate
    const failureRate =
      this.metrics.totalTests > 0 ? this.metrics.failedTests / this.metrics.totalTests : 0;
  ...
  monitorSystemResources() {
    setInterval(() => {
      ...
      this.log(
        `💾 Memory: ${(memUsage.heapUsed / 1024 / 1024).toFixed(2)}MB, Uptime: ${uptime.toFixed(0)}s`
      );
    ...
    return {
      ...
      failureRate:
        this.metrics.totalTests > 0
          ? ((this.metrics.failedTests / this.metrics.totalTests) * 100).toFixed(2) + "%"
          : "0%",
      duration: `${(duration / 1000).toFixed(2)}s`,
      avgTestDuration:
        this.metrics.testResults.length > 0
          ? `${(this.metrics.testResults.reduce((sum, test) => sum + test.duration, 0) / this.metrics.testResults.length).toFixed(0)}ms`
          : "0ms",
    };
  }
